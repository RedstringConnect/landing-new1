...
export function Navbar({ mapLayout = false }: NavbarProps) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/react-hooks-set-state-in-effect
    setMounted(true);
  }, []);
          ...
          {mobileOpen && (
            ...
            >
              <div className="mt-6 pt-4 border-t border-border flex flex-col">
                ...
                <motion.div variants={itemVariants} className="py-3 border-t border-border mt-2">
                ...
                <motion.div variants={itemVariants} className="py-3 border-t border-border">
  ...
  return (
    <>
      ...
      <div className="relative w-full h-0 md:h-10 pointer-events-none opacity-0" aria-hidden="true" />
                      ...
                      <li className="flex flex-col gap-1">
                        ...
                        <ul className="flex flex-col gap-1 w-full">
                          {contentLinks.map((link) => (
                            <li key={link.title}>
                              <NavigationMenuLink>
                                <Link href={link.href} className="flex flex-col items-start gap-2 p-3 h-auto">
                      ...
                      <li className="flex flex-col gap-1">
                        ...
                        <ul className="flex flex-col gap-1 w-full">
                          {hiringTools.map((tool) => (
                            <li key={tool.title}>
                              <NavigationMenuLink>
                                <Link href={tool.href} className="flex flex-col items-start gap-2 p-3 h-auto">
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className={`md:hidden overflow-hidden bg-background border border-border border-t-0 rounded-b-2xl ${isFloating ? "" : "mx-4"}`}
            >
                ...
                <div className="flex flex-col">
                  {navLinks.map((link) => (
                    <motion.div key={link.label} variants={itemVariants}>
                      <Link
                        href={link.href}
                        className={`block py-2.5 text-[15px] font-medium text-foreground hover:text-primary transition-colors`}
                        style={{ fontFeatureSettings: "'case', 'cv01', 'cv08', 'cv09', 'cv11', 'cv13'" }}
                        onClick={() => setMobileOpen(false)}
                        {...(link.href.startsWith("http") ? { target: "_blank", rel: "noopener noreferrer" } : {})}
                      >
                  ...
                  <motion.div variants={itemVariants} className="pt-4 pb-2">
                    <span className="text-[13px] text-muted-foreground font-semibold uppercase tracking-wider mb-1.5 block">Content & Guides</span>
                    <div className="flex flex-col pl-1">
                      {contentLinks.map((link) => (
                        <Link
                          key={link.title}
                          href={link.href}
                          className="block py-1.5 text-[14px] font-medium text-foreground hover:text-primary transition-colors"
                          onClick={() => setMobileOpen(false)}
                        >
                          {link.title}
                        </Link>
                  ...
                  <motion.div variants={itemVariants} className="pt-4 pb-2">
                    <span className="text-[13px] text-muted-foreground font-semibold uppercase tracking-wider mb-1.5 block">Hiring Tools</span>
                    <div className="flex flex-col pl-1">
                      {hiringTools.map((tool) => (
                        <Link
                          key={tool.title}
