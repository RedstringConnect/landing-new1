      ...
      <div className="relative w-full overflow-hidden border-t border-b border-border">
        <div className="flex animate-marquee-rtl">
          {doubled.map((logo, i) => (
            <div
              key={`${logo.name}-${i}`}
              className="flex items-center justify-center py-8 px-8 md:px-12 border-r border-border shrink-0"
            >
              <Image
                src={logo.src}
                alt={logo.name}
                width={120}
                height={120}
                className="w-[100px] h-[100px] md:w-[120px] md:h-[120px] object-contain brightness-105"
              />
            </div>
          ))}
        </div>
